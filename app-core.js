...
const PROJECTED_KNOCKOUT_MATCHES = [
    ...
    ...createProjectedKnockoutRound({
        ...
        placeholders: [
            ["Ganador 73", "Ganador 74"],
            ["Ganador 75", "Ganador 76"],
            ["Ganador 77", "Ganador 78"],
            ["Ganador 79", "Ganador 80"],
            ["Ganador 81", "Ganador 82"],
            ["Ganador 83", "Ganador 84"],
            ["Ganador 85", "Ganador 86"],
            ["Ganador 87", "Ganador 88"]
        ]
    }),
    ...createProjectedKnockoutRound({
...
function renderParticipants() {
    ...
    if (refs.participantStatus) {
        if (!hasParticipants) {
            refs.participantStatus.textContent = "No hay participantes registrados todavia. Pidele al admin que te cree.";
        } else if (!uiState.participantUnlocked) {
            refs.participantStatus.textContent = "Selecciona tu usuario y escribe tu PIN para entrar.";
        } else {
...
function renderMatches() {
    ...
    if (!fixtureTabs.length) {
        ...
        refs.matchesTableBody.innerHTML = '<div class="empty-state">Todavia no llegaron partidos desde FIFA. Usa el boton de sincronizacion.</div>';
        return;
    }
                    ...
                    return `
                        <article class="knockout-match">
                            ...
                            <div class="knockout-teams">
                                <div class="${rowClass}">
                                    ${renderTeamLabel(match.homeTeam, "knockout-team-name")}
                                ...
                                <div class="${rowClass}">
                                    ${renderTeamLabel(match.awayTeam, "knockout-team-name")}
                                    ${activeParticipant ? renderPredictionInput({
                                        matchId: match.id,
                                        side: "away",
                                        value: prediction.away,
...
function renderPredictionReviewTable() {
    ...
    if (!rows.length) {
        refs.predictionReviewTableBody.innerHTML = `<tr><td colspan="5" class="empty-state">${escapeHtml(participant.name)} todavia no tiene marcadores guardados.</td></tr>`;
...
function renderSpecialPredictions(activeParticipant, teams) {
    ...
    if (refs.specialPredictionsStatus) {
        if (!uiState.participantUnlocked || !participant) {
            refs.specialPredictionsStatus.textContent = "Inicia sesion para guardar tus picks extra.";
        } else {
...
function buildLeaderboardWorksheetXml(rows) {
    const headerCells = ["#", "Participante", "Jugador elegido", "Exactos", "Signos", "Bonus jugador", "Bonus equipo", "Puntos"];
    const sheetRows = [
        headerCells,
        ...rows.map((row) => [
            row.rank,
...
function formatSyncStatus() {
    ...
    return "Sin sincronizar todavia.";
}
