...
async function syncFifaMatches(options = {}) {
    ...
    try {
        ...
        const matchReplacementMap = buildMatchReplacementMap(state.matches, nextMatches);
        ...
        state.predictions = prunePredictions(state.predictions, allowedMatchIds, matchReplacementMap);
        state.participants = remapParticipantKnockoutSelections(state.participants, allowedMatchIds, matchReplacementMap);
...
function buildMatchReplacementMap(previousMatches, nextMatches) {
    const replacementMap = new Map();
    const previousMatchesList = Array.isArray(previousMatches) ? [...previousMatches] : [];
    const nextMatchesList = Array.isArray(nextMatches) ? [...nextMatches] : [];
    const officialKnockoutMatchesByNumber = new Map(
        nextMatchesList
            .filter((match) => !isProjectedMatch(match) && !match.group && match.matchNumber)
            .map((match) => [match.matchNumber, match])
    );
    const previousPhaseMatches = new Map();
    const nextPhaseMatches = new Map();

    for (const match of previousMatchesList
        .filter((entry) => isProjectedMatch(entry) && !entry.group)
        .sort(compareMatches)) {
        const phaseKey = getKnockoutPhaseKey(match);

        if (!phaseKey) {
            continue;
        }

        if (!previousPhaseMatches.has(phaseKey)) {
            previousPhaseMatches.set(phaseKey, []);
        }

        previousPhaseMatches.get(phaseKey).push(match);
    }
    ...
    for (const match of nextMatchesList
        ...
        if (!nextPhaseMatches.has(phaseKey)) {
            nextPhaseMatches.set(phaseKey, []);
        }

        nextPhaseMatches.get(phaseKey).push(match);
    }

    for (const oldMatch of previousMatchesList) {
        if (!isProjectedMatch(oldMatch) || oldMatch.group) {
            continue;
        }

        if (nextMatchesList.some((match) => match.id === oldMatch.id)) {
            continue;
        }

        let replacement = oldMatch.matchNumber
            ? officialKnockoutMatchesByNumber.get(oldMatch.matchNumber)
            : null;

        if (!replacement) {
            const phaseKey = getKnockoutPhaseKey(oldMatch);
            const previousPhaseList = previousPhaseMatches.get(phaseKey) || [];
            const nextPhaseList = nextPhaseMatches.get(phaseKey) || [];
            const phaseIndex = previousPhaseList.findIndex((match) => match.id === oldMatch.id);

            if (phaseIndex >= 0 && phaseIndex < nextPhaseList.length) {
                replacement = nextPhaseList[phaseIndex];
            }
        }

        if (replacement?.id) {
            replacementMap.set(oldMatch.id, replacement.id);
        }
    }
...
function mergePredictionValues(existingValue, incomingValue) {
    const existingScore = getScore(existingValue);
    const incomingScore = getScore(incomingValue);

    return {
        home: existingScore.home === "" ? incomingScore.home : existingScore.home,
        away: existingScore.away === "" ? incomingScore.away : existingScore.away
    };
}

function mergeKnockoutSelectionValues(existingValue, incomingValue) {
    const existingHomeTeam = String(existingValue?.homeTeam || "").trim();
    const existingAwayTeam = String(existingValue?.awayTeam || "").trim();
    const incomingHomeTeam = String(incomingValue?.homeTeam || "").trim();
    const incomingAwayTeam = String(incomingValue?.awayTeam || "").trim();

    return {
        homeTeam: existingHomeTeam || incomingHomeTeam,
        awayTeam: existingAwayTeam || incomingAwayTeam
    };
}

function remapParticipantKnockoutSelections(participants, allowedMatchIds, replacementMap = new Map()) {
    return (Array.isArray(participants) ? participants : []).map((participant) => {
        const nextSelections = {};

        for (const [matchId, selection] of Object.entries(participant?.knockoutSelections || {})) {
            const targetMatchId = allowedMatchIds.has(matchId) ? matchId : replacementMap.get(matchId);

            if (!targetMatchId) {
                continue;
            }

            nextSelections[targetMatchId] = mergeKnockoutSelectionValues(nextSelections[targetMatchId], selection);
        }

        return {
            ...participant,
            knockoutSelections: normalizeKnockoutSelections(nextSelections)
        };
    });
}
...
function prunePredictions(predictions, allowedMatchIds, replacementMap = new Map()) {
    const nextPredictions = {};

    for (const [participantId, participantPredictions] of Object.entries(predictions || {})) {
        const filteredEntries = {};

        for (const [matchId, prediction] of Object.entries(participantPredictions || {})) {
            const targetMatchId = allowedMatchIds.has(matchId) ? matchId : replacementMap.get(matchId);
            ...
            filteredEntries[targetMatchId] = mergePredictionValues(filteredEntries[targetMatchId], prediction);
        }

        if (Object.keys(filteredEntries).length) {
            nextPredictions[participantId] = filteredEntries;
        }
    }
