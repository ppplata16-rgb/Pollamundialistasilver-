...
def is_complete_score(value) -> bool:
    return (
        isinstance(value, dict)
        and isinstance(value.get("home"), int)
        and value.get("home") >= 0
        and isinstance(value.get("away"), int)
        and value.get("away") >= 0
    )


def merge_protected_state(existing: dict, incoming: dict) -> dict:
    ...
    existing_locked = existing.get("lockedMatches") if isinstance(existing.get("lockedMatches"), dict) else {}
    incoming_locked = incoming.get("lockedMatches") if isinstance(incoming.get("lockedMatches"), dict) else {}
    existing_predictions = existing.get("predictions") if isinstance(existing.get("predictions"), dict) else {}
    incoming_predictions = incoming.get("predictions") if isinstance(incoming.get("predictions"), dict) else {}

    protected_results = dict(incoming_results)
    protected_locked = dict(incoming_locked)
    protected_predictions = {
        participant_id: dict(predictions)
        for participant_id, predictions in incoming_predictions.items()
        if isinstance(predictions, dict)
    }

    for match_id, result in existing_results.items():
        if existing_locked.get(match_id) or is_complete_score(result):
            protected_results[match_id] = result
            protected_locked[match_id] = True

    for match_id, is_locked in existing_locked.items():
        if is_locked:
            protected_locked[match_id] = True

    locked_match_ids = {match_id for match_id, is_locked in protected_locked.items() if is_locked}

    for participant_id, participant_predictions in protected_predictions.items():
        for match_id in locked_match_ids:
            if match_id in participant_predictions:
                participant_predictions.pop(match_id, None)

    for participant_id, participant_predictions in existing_predictions.items():
        if not isinstance(participant_predictions, dict):
            continue

        protected_predictions.setdefault(participant_id, {})

        for match_id in locked_match_ids:
            if match_id in participant_predictions:
                protected_predictions[participant_id][match_id] = participant_predictions[match_id]

    merged["results"] = protected_results
    merged["lockedMatches"] = protected_locked
    merged["predictions"] = {
        participant_id: predictions
        for participant_id, predictions in protected_predictions.items()
        if predictions
    }
    return merged
...
def save_state(state: dict) -> dict:
    state = merge_protected_state(load_state(), state)
